...
// Only needed for dynamic load balancing
...
//    MPI_Aint offsets[2];
//
//    offsets[0] = offsetof(stack_data, Item);
...
void MPI_Wrapper_Send_Problem(stack_data node, int destination) {
    ...
    MPI_Send(node.taken, _problemSize, MPI_INT, destination, MPI_PROBLEM_TAG, MPI_COMM_WORLD);
}

stack_data MPI_Wrapper_Receive_Problem() {

    stack_data node;
    node.taken = malloc(_problemSize * sizeof(int));

    MPI_Recv(&node, 1, _stackDataType, _rank, MPI_PROBLEM_TAG, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    MPI_Recv(node.taken, _problemSize, MPI_INT, _rank, MPI_PROBLEM_TAG, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
...
void MPI_Wrapper_Receive_Bound(stack_data *bound) {
    ...
    if (flag) {
        MPI_Status status;
        int externalBound;
        MPI_Recv(&externalBound, 1, MPI_INT, probeStatus.MPI_SOURCE, MPI_BOUND_TAG, MPI_COMM_WORLD, &status);
        if (bound->price <= externalBound) {
            bound->price = externalBound;
...
void MPI_Wrapper_Share_Bound(stack_data bound) {
    for (int destination = 0; destination < _size; ++destination) {
        ...
        MPI_Send(&bound.price, 1, MPI_INT, destination, MPI_BOUND_TAG, MPI_COMM_WORLD);
    }
...
void MPI_Wrapper_Synchronize(stack_data *bound) {
    ...
    MPI_Bcast(bound->taken, _problemSize, MPI_INT, boundSource, MPI_COMM_WORLD);
}
